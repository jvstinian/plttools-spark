...
class ELTRollup extends FixtureAnyFlatSpec {
    ...
    def withFixture(test: OneArgTest): Outcome = {
        ...
        val rates: Seq[Double] = Seq(0.000018, 0.00021, 0.000305, 0.000912, 0.000078, 0.000042, 0.000018, 0.00021, 0.000411, 0.000912, 0.000729)
        ...
        val records: Seq[ELTRecord] = eventIds.zip(rates).zip(losses).zip(sdis).zip(sdcs).zip(expvals).map{ case (((((eventId, rate), loss), sdi), sdc), expval) =>
