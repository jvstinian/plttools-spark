...
class PLTCalculatorSpec extends FixtureAnyFlatSpec {
  ...
  "Test OEP calculations" should "all have EPType \"OEP\"" in { fixture =>
    val oepdf = PLT.calculateOEPForReturnPeriodsBySubportfolio(
      fixture.data,
      EPCurve.RETURN_PERIODS.toSeq.map(_.toDouble),
      Some(1.0 / 5.0)
    )
    val isOep =
  ...
  "Test AEP calculations" should "all have EPType \"AEP\"" in { fixture =>
    val aepdf = PLT.calculateAEPForReturnPeriodsBySubportfolio(
      fixture.data,
      EPCurve.RETURN_PERIODS.toSeq.map(_.toDouble),
      Some(1.0 / 5.0)
    )
