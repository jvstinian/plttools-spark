...
object ELT {
  def rollUp(eltdf: DataFrame): DataFrame = {
    ...
    val aggdf = df
      .groupBy("SubportfolioId", "EventId")
      .agg(
        max(col("Rate")).alias("Rate"),
  ...
  def calculateStatistics(eltdf: DataFrame): DataFrame = this
    .rollUp(eltdf)
    ...
    .agg(sum(col("Rate") * col("Loss")).alias("AAL"), sum(col("VarianceTerm")).alias("Variance"))
