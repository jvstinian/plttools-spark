...
final case class PLTRecordParseException(
    private val message: String = "", private val cause: Throwable = None.orNull)
...
object PLTRecordParser {
    ...
    def parseRecord(record: String): PLTRecord = {
        ...
        record.split(',') match {
            case Array(lossType, periodIdStr, weightStr, eventIdStr, eventDateStr, lossDateStr, lossStr) => PLTRecord(
                lossType, periodIdStr.toInt, weightStr.toDouble, /*BigInt(eventIdStr)*/eventIdStr.toLong, 
...
object RMSPLTAnalyticsExample {
    def main(args: Array[String]): Unit = { 
        ...
        val pltrecs = Source.fromInputStream(getClass.getResourceAsStream("/plt.csv")).getLines().toSeq.tail.map(PLTRecordParser.parseRecord)
